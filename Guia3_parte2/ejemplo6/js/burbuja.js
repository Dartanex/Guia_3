function inicializar(){
    ...
    do{
        ...
        //Verificar que se ingresa un dato numérico
        if(isNaN(max)){
            alert("El valor digitado no es numérico");
            continue;
        }
        //Verificar que el valor ingresado sea igual o mayor a 2
        if(max < 2){
            alert("El arreglo debe ser de dimensión 2 o mayor");
        }
    ...
    //Obteniendo el elemento donde se cargará el contenido generado dinámicamente desde JavaScript
    ...
    with(document){
        ...
        contenido += "<table>\n\t<tbody>\n\t\t<tr>\n";
        //Lazo ue muestra los elementos del arreglo que han sido ordenados con el método de la burbuja
        for(i=0;i<max;i++){
            contenido += "\t\t\t<td class=\"Off\">" + numeros[i] + "</td>\n";
        }
        contenido += "\t\t</tr>\n\t</tbody>\n</table>\n";
